# from core.config.production_config import production_CONFIG, production_INDEX
...
def get_production_config():
    ...
    if production_CONFIG is None:
        with open("conf/production.yaml") as f:
            yaml_as_dict = yaml.load(f)
            production_CONFIG = yaml_as_dict
...
def get_root_nodes():
    return [node for node in get_production_config().values() if "root" in node and node["root"]]


def get_nodes():
    return [node for node in get_production_config().values()]


def get_node_by_id(node_id):
    production_index = get_production_index()
    if node_id in production_index:
        return production_index[node_id]

    candidates = [node for node in get_production_config().values() if node["id"] == node_id]
    if len(candidates) > 0:
        return candidates[0]
    return None
...
def get_datacenter_weighted_tree_production_data():
    datacenter_root_node = get_node_by_id("overall_production")
